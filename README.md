...
- Use Helm to install an application (e.g., Apache or Redis). Share the command and verify it was deployed. (5 points)
...
The values.yaml file holds the configuration for a chart, meaning the default settings used by the chart at deployment (such as image tags, resource limits, service types) resulting in a simpler deployment process. This values can be customized and overwritten at runtime (via ```--set``` or a custom values file) to better fit the application needs. Moreover, the chart is reusable and adaptable across differnt environments.
...
Once the load increases the HPA comes into action.
...
- application crashes at startup due to an error in the container's main process, script/command/entrypoint error
...
These are EC2 instances managed by EKS to run your workloads, and you have control over instance types, scaling, and AMIs.

The worker nodes are part of an Auto Scaling Group, and you are responsible for some aspects of node management (patching, capacity planning).

Fargate Profiles:
Serverless compute for EKS; you don’t manage EC2 instances at all.
Pods matching the profile run on Fargate, and AWS handles the provisioning, scaling, and patching.

Great for workloads where you want to avoid managing infrastructure or for unpredictable workloads.

So, managed node groups give you control over the underlying EC2 instances, while Fargate abstracts the nodes completely and runs pods in a serverless fashion.
